...
def url2options(url):
    """ Get pusher options from url.

    Url has following format:
    http://xxxxxxxxxxxxxxxxxxxx:yyyyyyyyyyyyyyyyyy@api.pusherapp.com/apps/zzzzz
    ...
    app_id = parsed_url.path.rsplit("/", 1)[-1]
...
class Channel(object):
    def __init__(self, name, pusher):
        ...
        self.path = '/apps/%s/channels/%s/events' % (
            self.pusher.app_id, urllib.quote(self.name))
    ...
    def trigger(self, event, data={}, socket_id=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT):
        ...
        status, resp_content = self.send_request(
            signed_path, json_data, timeout=timeout)
        if status == 202:
            return True
        elif status == 401:
            raise AuthenticationError(
                "Status: 401; Message: %s" % resp_content)
        elif status == 404:
            raise NotFoundError("Status: 404; Message: %s" % resp_content)
        elif status == 403:
            raise AppDisabledOrMessageQuotaError(
                "Status: 403; Message: %s" % resp_content)
        else:
            raise UnexpectedReturnStatusError(
                "Status: %s; Message: %s" % (status, resp_content))
    ...
    def signed_query(self, event, json_data, socket_id):
        ...
        signature = hmac.new(
            self.pusher.secret, string_to_sign, hashlib.sha256).hexdigest()
